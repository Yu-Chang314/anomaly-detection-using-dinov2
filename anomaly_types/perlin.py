...
def generate_perlin_noise(
    ...
    def dot(grad: torch.Tensor, shift: list[float]) -> torch.Tensor:
        return (
            torch.stack(
                (
                    grid[:pad_h, :pad_w, 0] + shift[0],
                    grid[:pad_h, :pad_w, 1] + shift[1],
                ),
                dim=-1,
            )
            * grad[:pad_h, :pad_w]
...
class PerlinAnomalyGenerator(v2.Transform):
    def __init__(
        ...
        self.augmenters = MultiRandomChoice(
            transforms=[
                ...
                v2.RandomSolarize(
                    threshold=torch.empty(1).uniform_(32 / 255, 128 / 255).item(), p=1.0
                ),
    ...
    def generate_perturbation(
        ...
        if not (perlin_noise > 0.5).any():
            perlin_noise = (perlin_noise - perlin_noise.min()) / (
        ...
        if anomaly_source_path:
            anomaly_source_img = (
                io.read_image(str(anomaly_source_path), mode=io.ImageReadMode.RGB)
                .float()
                .to(device)
                / 255.0
            )
            if anomaly_source_img.shape[-2:] != (height, width):
                anomaly_source_img = v2.functional.resize(
                    anomaly_source_img, [height, width], antialias=True
                )
            anomaly_source_img = anomaly_source_img.permute(1, 2, 0)  # [H, W, C]
        else:
            anomaly_source_img = perlin_noise.unsqueeze(-1).repeat(1, 1, 3)  # [H, W, C]
            anomaly_source_img = (
                anomaly_source_img * 0.5
            ) + 0.25  # Adjust intensity range

        # Apply augmentations to source image
        anomaly_augmented = self.augmenters(
            anomaly_source_img.permute(2, 0, 1)
        )  # [C, H, W]
    ...
    def _transform_image(
        ...
        anomaly_source_path = (
            list(self.anomaly_source_paths)[
                int(torch.randint(len(self.anomaly_source_paths), (1,)).item())
            ]
            if self.anomaly_source_paths
            else None
        )

        perturbation, mask = self.generate_perturbation(
            h, w, device, anomaly_source_path
        )
        ...
        beta = (
            ...
            if isinstance(self.blend_factor, float)
                ...
                * (self.blend_factor[1] - self.blend_factor[0])
                + self.blend_factor[0]
